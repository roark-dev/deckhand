...
func render(st broker.Status) string {
	...
	counter("deckhand_job_queue_seconds_sum", "total seconds jobs spent queued before runner assignment", st.Counters.QueueMsSum/1000)
	counter("deckhand_job_queue_seconds_count", "jobs measured for queue latency", st.Counters.QueueCount)
	counter("deckhand_job_duration_seconds_sum", "total job run seconds (assigned to finished)", st.Counters.DurationMsSum/1000)
	counter("deckhand_job_duration_seconds_count", "jobs measured for duration", st.Counters.DurationCount)
	gauge("deckhand_job_duration_seconds_min", "shortest job duration since daemon start (contention-variance floor)", st.Counters.DurationMsMin/1000)
	gauge("deckhand_job_duration_seconds_max", "longest job duration since daemon start (contention-variance ceiling)", st.Counters.DurationMsMax/1000)
